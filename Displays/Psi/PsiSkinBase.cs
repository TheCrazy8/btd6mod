...
public abstract class PsiSkinBase : ModCustomDisplay
{
    ...
    public override void ModifyDisplayNode(UnityDisplayNode node)
    {
        ...
        // Idle animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 0, 0, 0, idleClip, InterruptBehaviour.Higher);

        // Attack animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 1, 1, 1, attackClip);

        // Psychic Blast animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 2, 2, 3, psychicBlastClip);
        
        // Psychic Blast animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 3, 3, 2, psionicScreamClip);

        // Upgrade animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 4, 4, 2, upgradeClip);
        
        // Idle again. This doesn't make sense, but this is what Psi animations have.
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 5, 5, 0, idleClip, InterruptBehaviour.Higher);

        // Falling animation
        BTD6Mod.ConfigureAnimationBaker(animationController.placementConfigs, 0, 6, 5, fallClip,
            InterruptBehaviour.NoInterrupt);
        
        // Attack idle animation
        BTD6Mod.ConfigureAnimationBaker(animationController.attackIdleConfigs, 0, 7, 0, attackIdleClip,
            InterruptBehaviour.Higher);

        // Idle variant 1
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 0, 8, 0, idle1Clip);

        // Idle variant 2
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 1, 9, 0, idle2Clip);
        
        // Idle variant 3
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 2, 10, 0, idle3Clip);
    }
}
