...
public abstract class QuincySkinBase : ModCustomDisplay
{
    ...
    public override void ModifyDisplayNode(UnityDisplayNode node)
    {
        ...
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 2, 2, 3, specialAttackClip,
            InterruptBehaviour.NoInterrupt);

        // Upgrade animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 3, 3, 2, upgradeClip,
            InterruptBehaviour.Higher);
        
        // Falling animation
        BTD6Mod.ConfigureAnimationBaker(animationController.animationStates, 4, 4, 5, fallClip,
            InterruptBehaviour.NoInterrupt);
        
        // Attack idle animation
        BTD6Mod.ConfigureAnimationBaker(animationController.attackIdleConfigs, 0, 5, 0, attackIdleClip,
            InterruptBehaviour.Higher);

        // Idle variant 1
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 0, 6, 0, idle1Clip);

        // Idle variant 2
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 1, 7, 0, idle2Clip);
        
        // Idle variant 3
        BTD6Mod.ConfigureAnimationBaker(animationController.IdleVariants, 2, 8, 0, idle3Clip);
    }
}
